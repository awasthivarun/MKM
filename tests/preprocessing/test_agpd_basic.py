...
def test_load_agpd_workbook(config):
    path = (
        "data/raw/AgPd_COOx_basic/"
        "Ag10Pd90_current_densities.xlsx"
    )
    ...
    assert set(df["C_KOH_M"]) == {
        0.25,
        0.50,
        1.00,
    }

    assert set(df["CO_mole_fraction"]) == {
        0.001,
        0.01,
        0.10,
        1.00,
...
def test_workbook_has_expected_number_of_curves(config):
    ...
    expected_curves = (
        len(config["KOH_concentrations_M"])
        * len(config["CO_mole_fractions"])
        * len(config["replicates"])
    )
    ...
    assert np.all(
        curve_sizes.to_numpy()
        == config["potential"]["expected_points"]
    )
...
def test_current_reconstruction(config):
    ...
    reconstructed = np.exp(
        df["ln_j_uA_cm2"].to_numpy()
    )

    np.testing.assert_allclose(
        df["j_uA_cm2"].to_numpy(),
        reconstructed,
    )
...
def test_build_agpd_analysis_grid(config):
    ...
    analysis = build_agpd_analysis_grid(
        standardized,
        config,
    )
    ...
    assert set(analysis["material"]) == {
        "Ag10Pd90"
    }

    assert set(analysis["C_KOH_M"]) == {
        0.25,
        0.50,
        1.00,
    }

    assert set(analysis["CO_mole_fraction"]) == {
        0.001,
        0.01,
        0.10,
        1.00,
...
def test_analysis_grid_spacing(config):
    ...
    for _, curve in analysis.groupby(
        condition_columns,
        sort=False,
    ):
...
def test_analysis_grid_is_shared_across_replicates(config):
    ...
    for _, condition in analysis.groupby(
        condition_columns,
        sort=False,
    ):
        replicate_grids = []
        ...
        for replicate in config["replicates"]:
            replicate_df = condition[
                condition["replicate"] == replicate
            ]

            replicate_grids.append(
...
def test_analysis_grid_stays_inside_raw_replicate_domains(config):
    ...
    raw_groups = standardized.groupby(
        condition_columns,
        sort=False,
    )

    analysis_groups = analysis.groupby(
        condition_columns,
        sort=False,
    )
...
def test_analysis_grid_matches_global_lattice(config):
    ...
    expected_potential = (
        origin
        + analysis["analysis_grid_index"].to_numpy()
        * spacing
    )

    np.testing.assert_allclose(
        analysis["E_V_SHE"].to_numpy(),
        expected_potential,
        rtol=0,
        atol=1e-12,
    )
...
def test_analysis_grid_current_reconstruction(config):
    ...
    reconstructed = np.exp(
        analysis["ln_j_uA_cm2"].to_numpy()
    )

    np.testing.assert_allclose(
        analysis["j_uA_cm2"].to_numpy(),
        reconstructed,
    )

    assert np.all(
        analysis["j_uA_cm2"].to_numpy() > 0
    )
...
def test_analysis_grid_has_unique_rows(config):
    ...
    assert not analysis.duplicated(
        key_columns
    ).any()
...
def test_agpd_rate_normalization(config):
    ...
    expected_rate = (
        rates["j_uA_cm2"].to_numpy()
        / denominator
    )

    np.testing.assert_allclose(
        rates["rate_s_inv"].to_numpy(),
        expected_rate,
    )
...
def test_agpd_log_rate_normalization(config):
    ...
    expected_log_rate = (
        rates["ln_j_uA_cm2"].to_numpy()
        - np.log(denominator)
    )

    np.testing.assert_allclose(
        rates["ln_rate"].to_numpy(),
        expected_log_rate,
    )
...
def test_rate_and_log_rate_are_consistent(config):
    ...
    np.testing.assert_allclose(
        np.log(rates["rate_s_inv"].to_numpy()),
        rates["ln_rate"].to_numpy(),
    )
...
def test_agpd_replicate_summary(config):
    ...
    summary = summarize_agpd_replicates(
        rates,
        config,
    )
...
def test_agpd_replicate_means(config):
    ...
    np.testing.assert_allclose(
        summary["rate_mean_s_inv"],
        expected["rate_mean"],
    )

    np.testing.assert_allclose(
        summary["ln_rate_mean"],
        expected["ln_rate_mean"],
    )
...
def test_rate_sd_scales_with_current_sd(config):
    ...
    np.testing.assert_allclose(
        summary["rate_sd_s_inv"],
        summary["j_sd_uA_cm2"] / denominator,
    )
...
def test_agpd_transfer_coefficients(config):
    ...
    standardized = load_agpd_workbook(
        path,
        config,
    )
    ...
    rates_with_alpha = add_agpd_transfer_coefficients(
        rates,
        config,
    )
...
def test_agpd_transfer_coefficients_do_not_change_rate_data(config):
    ...
    for column in columns:
        np.testing.assert_array_equal(
            rates_with_alpha[column].to_numpy(),
...
def test_agpd_summary_contains_alpha_statistics(config):
    ...
    rates = add_agpd_transfer_coefficients(
        rates,
        config,
    )
...
def test_agpd_alpha_summary_matches_replicate_data(config):
    ...
    expected = (
        ...
        .agg(
            alpha_mean_expected=("alpha", "mean"),
            alpha_sd_expected=("alpha", "std"),
        )
        .reset_index()
    )
    ...
    np.testing.assert_allclose(
        summary["alpha_mean"].to_numpy(),
        expected["alpha_mean_expected"].to_numpy(),
    )

    np.testing.assert_allclose(
        summary["alpha_sd"].to_numpy(),
        expected["alpha_sd_expected"].to_numpy(),
    )
...
def test_low_rate_truncation_uses_sustained_pre_peak_crossing(config):
    ...
    truncated, cutoffs = truncate_agpd_analysis(
        data,
        test_config,
    )
...
def test_post_peak_rate_below_threshold_is_retained(config):
    ...
    post_peak = truncated[
        truncated["E_V_SHE"] == 0.04
    ]

    assert len(post_peak) == 3

    assert np.all(
        post_peak["rate_s_inv"].to_numpy()
        < 1.0e-3
    )
...
def test_truncation_produces_contiguous_potential_window(config):
    ...
    truncated, _ = truncate_agpd_analysis(
        data,
        test_config,
    )
    ...
    for replicate in [
        ...
        curve = (
            truncated[
                truncated["replicate"] == replicate
            ]
            .sort_values("E_V_SHE")
        )

        np.testing.assert_array_equal(
            curve["E_V_SHE"].to_numpy(),
            expected_potential,
        )
...
def test_truncation_records_condition_provenance(config):
    ...
    _, cutoffs = truncate_agpd_analysis(
        data,
        test_config,
    )
...
def test_real_agpd_truncation_satisfies_low_potential_rule(config):
    ...
    for condition, cutoff in cutoffs.groupby(
        condition_columns,
        sort=False,
    ):
        ...
        condition_data = truncated[
            ...
            & (
                truncated["CO_mole_fraction"]
                == condition[2]
            )
        ]

        pre_peak = condition_data[
            condition_data["E_V_SHE"]
            <= cutoff["peak_E_V_SHE"]
        ]
...
def _build_selected_agpd_summary(path, config):
    ...
    analysis = add_agpd_rates(
        analysis,
        config,
    )

    selected, _ = truncate_agpd_analysis(
        analysis,
        config,
    )

    selected = add_agpd_transfer_coefficients(
        selected,
        config,
    )

    return summarize_agpd_replicates(
        selected,
        config,
    )
...
def test_agpd_oh_order_structure(config):
    ...
    summary = _build_selected_agpd_summary(
        path,
        config,
    )

    delta_OH = calculate_agpd_oh_order(
        summary,
        config,
    )
    ...
    assert set(
        delta_OH["CO_mole_fraction"]
    ) == set(
        config["CO_mole_fractions"]
    )

    assert set(
        delta_OH["n_order_conditions"]
    ) == {
        len(config["KOH_concentrations_M"])
    }
