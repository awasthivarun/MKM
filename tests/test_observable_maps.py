...
def test_alpha_map_recovers_linear_log_rate():
    ...
    alpha = evaluate_linear_observable_map(
        log_rate=log_rate,
...
def test_log_slope_map_recovers_known_order():
    ...
    order = evaluate_linear_observable_map(
        log_rate,
        order_map,
    )
...
def test_adjacent_order_map_has_three_co_ranges():
    ...
    pairs = set(
        zip(
            order_map.outputs["CO_lower"],
            order_map.outputs["CO_upper"],
        )
    )
