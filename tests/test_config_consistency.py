...
def test_pd100_reduced_model_prior_matches_reduced_parameter_set():
    ...
    config = _load_yaml("config/models/agpd_basic.yaml")
    profile = get_agpd_prior_profile(config, "Pd100", "CO_ER_LH")
    assert set(profile["parameters"]) == {
        field.name for field in fields(PdCOERLHParameters)
    }
...
def test_first_composition_parameterization_batch_has_expected_slope_sets():
    ...
    expected = {
        ...
        "linear_thermo": {"deltaG1_0", "deltaG4_0", "deltaG5_0"},
...
def test_same_parameter_uses_same_slope_prior_across_profiles():
    config = _load_yaml("config/models/agpd_basic.yaml")
    profiles = config["composition_parameterizations"]
    anchor_slopes = profiles["linear_selected_energies"]["models"]["CO_BF_ER_LH"]["slopes"]

    comparisons = {
        "deltaG1_0": ("linear_dG1", "linear_thermo"),
        "deltaG4_0": ("linear_dG4", "linear_thermo"),
        "deltaG5_0": ("linear_dG5", "linear_thermo"),
        "Gact2_BF_0": ("linear_GactBF", "linear_oxidation_barriers"),
        "Gact2_ER_0": ("linear_GactER", "linear_oxidation_barriers"),
    }

    for parameter_name, profile_names in comparisons.items():
        expected_prior = anchor_slopes[parameter_name]
        for profile_name in profile_names:
            actual_prior = profiles[profile_name]["models"]["CO_BF_ER_LH"]["slopes"][parameter_name]
            assert actual_prior == expected_prior


def test_full_linear_xag_uses_current_broad_slope_priors():
    config = _load_yaml("config/models/agpd_basic.yaml")
    slopes = config["composition_parameterizations"]["linear_xAg"]["models"]["CO_BF_ER_LH"]["slopes"]
    ...
    for parameter_name in energy_parameters:
    ...
    for parameter_name in {"beta_2_BF", "beta_2_ER", "q"}:
        prior = slopes[parameter_name]
        assert prior == {
            "distribution": "normal",
            "mu": 0.0,
