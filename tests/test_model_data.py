...
def test_model_data_preserves_only_observed_conditions():
    ...
    result = build_model_data(selected_replicates=data, electrolyte_concentration_column="C_KOH_M")

    assert len(result.conditions) == 2

    observed_conditions = set(
        zip(result.conditions["electrolyte_concentration_M"], result.conditions["CO_mole_fraction"])
    )

    assert observed_conditions == {(0.25, 0.001), (1.00, 0.10)}
...
def test_model_data_separates_points_from_replicates():
    ...
    assert len(result.model_points) == 3
...
def test_replicates_share_model_point():
    ...
    point = result.model_points[
        (result.model_points["electrolyte_concentration_M"] == 0.25)
        & (result.model_points["CO_mole_fraction"] == 0.001)
        & (result.model_points["analysis_grid_index"] == 0)
    ].iloc[0]

    observations = result.observations[result.observations["model_point_id"] == point["model_point_id"]]

    assert len(observations) == 3
    assert set(observations["replicate"]) == {"A", "B", "C"}
...
def test_model_data_allows_variable_replicate_counts():
    ...
    replicate_counts = result.observations.groupby("model_point_id").size().sort_values().to_numpy()

    np.testing.assert_array_equal(replicate_counts, np.array([1, 3, 3]))
