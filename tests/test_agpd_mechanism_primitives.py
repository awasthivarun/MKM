...
def test_zero_free_energy_has_unit_equilibrium_constant():
    log_K = log_equilibrium_constant(
        delta_G_eV=0.0,
        temperature_K=293.15,
    )
...
def test_electrochemical_free_energy_decreases_with_potential():
    ...
    delta_G = electrochemical_free_energy(
        delta_G_0_eV=0.2,
...
def test_pd_qea_coverages_satisfy_site_balance():
    ...
    theta_empty = pt.exp(
        coverages.log_theta_empty_Pd
    )

    theta_CO = pt.exp(
        coverages.log_theta_CO
    )

    theta_OH = pt.exp(
        coverages.log_theta_OH_Pd
    )
...
def test_ag_qea_coverages_satisfy_site_balance():
    ...
    coverages = calculate_ag_qea_coverages(
        log_K_OH_Ag=pt.as_tensor_variable(
            np.array([
                0.5,
                0.5,
                0.5,
    ...
    theta_empty = pt.exp(
        coverages.log_theta_empty_Ag
    )

    theta_OH = pt.exp(
        coverages.log_theta_OH_Ag
    )
