...
def agpd_contract_data():
    ...
    mean_log_rate = (
        ...
        .groupby("model_point_id")["ln_rate"]
        .mean()
        .reindex(
...
def test_real_agpd_model_data_contract(
    agpd_contract_data,
):
    selected = agpd_contract_data[
        "selected"
    ]

    model_data = agpd_contract_data[
        "model_data"
    ]

    assert (
        len(model_data.observations)
        == len(selected)
    )
    ...
    assert (
        len(model_data.conditions)
        == 12
    )


def test_alpha_map_matches_preprocessing_alpha(
    agpd_contract_data,
):
    config = agpd_contract_data[
        "config"
    ]

    summary = agpd_contract_data[
        "summary"
    ]
    ...
    mean_log_rate = agpd_contract_data[
        "mean_log_rate"
    ]
    ...
    mapped_alpha["alpha_mapped"] = (
        evaluate_linear_observable_map(
            log_rate=mean_log_rate,
            observable_map=alpha_map,
        )
    )
    ...
    np.testing.assert_allclose(
        comparison["alpha_mapped"],
        comparison["alpha_mean"],
        rtol=1e-10,
        atol=1e-10,
    )


def test_oh_order_map_matches_preprocessing(
    agpd_contract_data,
):
    config = agpd_contract_data[
        "config"
    ...
    expected = calculate_agpd_oh_order(
        summary,
        config,
    )
    ...
    order_map = build_log_slope_order_map(
        ...
        varying_values=config[
            "KOH_concentrations_M"
        ],
        group_columns=[
            "material",
            "CO_mole_fraction",
...
def test_co_order_map_matches_preprocessing(
    agpd_contract_data,
):
    config = agpd_contract_data[
        "config"
    ...
    expected = calculate_agpd_co_order(
        summary,
        config,
    )
