...
def test_bf_rate_is_finite():
    state = build_agpd_point_state(
        ...
        config=_make_alloy_config(),
    )

    result = evaluate_agpd_bf(
        state=state,
...
def test_bf_site_balances():
    ...
    theta_OH_Pd = pointwise[
        "theta_OH_Pd"
    ].eval()

    theta_empty_Pd = pointwise[
        "theta_empty_Pd"
    ].eval()

    theta_OH_Ag = pointwise[
        "theta_OH_Ag"
    ].eval()

    theta_empty_Ag = pointwise[
        "theta_empty_Ag"
    ].eval()
...
def test_bf_rate_scales_with_ag_fraction_at_fixed_state():
    ...
    result_10 = evaluate_agpd_bf(
        state=state_10,
        parameters=_make_parameters(),
        temperature_K=293.15,
    )

    result_50 = evaluate_agpd_bf(
        state=state_50,
        parameters=_make_parameters(),
        temperature_K=293.15,
    )
    ...
    ln_rate_10 = (
        result_10.log_rate_BF.eval()
    )

    ln_rate_50 = (
        result_50.log_rate_BF.eval()
    )
