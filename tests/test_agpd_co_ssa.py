...
def test_co_ssa_coverages_are_physical():
    ...
    result = evaluate_agpd_co_bf_er_lh(
        state=state,
...
def test_co_ssa_site_balances():
    ...
    Ag_total = (
        pointwise["theta_OH_Ag"].eval()
        + pointwise["theta_empty_Ag"].eval()
    )
...
def test_co_ssa_satisfies_steady_state_balance():
    ...
    log_k1 = (
        result.log_k1.eval()
        if hasattr(result.log_k1, "eval")
        else np.asarray(result.log_k1)
    )
    log_k_minus_1 = result.log_k_minus_1.eval()
    log_k2_BF = result.log_k2_BF.eval()
    log_k2_ER = result.log_k2_ER.eval()
    log_k2_LH = (
        result.log_k2_LH.eval()
        if hasattr(result.log_k2_LH, "eval")
        else np.asarray(result.log_k2_LH)
    )

    forward_CO = (
        np.exp(log_k1)
        * np.exp(state.ln_a_CO)
        * theta_empty
    )
...
def test_co_ssa_approaches_co_qea_when_consumption_is_slow():
    ...
    theta_CO_ssa = (
        result.mechanism_result.pointwise["theta_CO"].eval()
    )
...
def test_capped_co_ssa_limits_molecular_co_coverage():
    state = build_agpd_point_state(
        inputs=_make_inputs(),
        config=_make_config(theta_CO_max=0.66),
    )
    result = evaluate_agpd_co_bf_er_lh_capped(
        state=state,
        parameters=_make_parameters(),
        temperature_K=293.15,
    )
    ...
    theta_CO_site_occupation = (
        pointwise["theta_CO_site_occupation"].eval()
    )
...
def test_capped_co_ssa_uses_weighted_pd_site_balance():
    state = build_agpd_point_state(
        inputs=_make_inputs(),
        config=_make_config(theta_CO_max=0.66),
    )
...
def test_capped_model_with_unit_cap_matches_uncapped_model():
    state = build_agpd_point_state(
        inputs=_make_inputs(),
        config=_make_config(theta_CO_max=1.0),
    )
    parameters = _make_parameters()

    uncapped = evaluate_agpd_co_bf_er_lh(
        state=state,
        parameters=parameters,
        temperature_K=293.15,
    )
    capped = evaluate_agpd_co_bf_er_lh_capped(
        state=state,
        parameters=parameters,
        temperature_K=293.15,
    )
    ...
    np.testing.assert_allclose(
        capped.log_rate_total.eval(),
        uncapped.log_rate_total.eval(),
        rtol=1e-12,
        atol=1e-12,
    )
    ...
    for name in (
        ...
        np.testing.assert_allclose(
            capped.mechanism_result.pointwise[name].eval(),
            uncapped.mechanism_result.pointwise[name].eval(),
            rtol=1e-12,
            atol=1e-12,
        )


def test_capped_co_ssa_approaches_weighted_qea_when_consumption_is_slow():
    theta_CO_max = 0.66
    state = build_agpd_point_state(
        inputs=_make_inputs(),
        config=_make_config(theta_CO_max=theta_CO_max),
    )
