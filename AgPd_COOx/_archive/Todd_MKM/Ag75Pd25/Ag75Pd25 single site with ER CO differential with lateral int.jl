...
function controlFitting(global_opt_iter, local_opt_iter, priors, bounds)
    ...
    beta_ER_bounds = bounds[3]
    z_CO_CO_bounds = bounds[4]
    z_CO_OH_bounds = bounds[5]
    z_OH_CO_bounds = bounds[6]
    z_OH_OH_bounds = bounds[7]

    function simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array, P_CO_array, a_OH_array, orderType)
        ...
        if orderType == "CO order"
            ...
            for i = 1:length(a_OH_array)
                ...
                k_ER_fwd = zeros(length(E_array))
                ddG_ER = zeros(length(P_CO_array),length(E_array))
                k_ER_fwd_cov_dep = zeros(length(P_CO_array),length(E_array))
                ER_rate = zeros(length(P_CO_array),length(E_array))
                ...
                for j = 1:length(P_CO_array)
                    for k = 1:length(E_array)
                        ...
                        if alpha_BEP + gamma_BEP * (0.67 - 0.25 * G0_CO_ref - G0_OH_ref) < 0
                            ...
                            dG_ER_act_ref = alpha_BEP + gamma_BEP * (0.67 - 0.25 * G0_CO_ref - G0_OH_ref)
                        end

                        #define potential dependent barriers at zero coverage
                        if dG_ER_act_ref - beta_ER * (E_array[k] - G0_OH_ref) < 0
                            dG_ER_act = 0
                        else
                            dG_ER_act = dG_ER_act_ref - beta_ER * (E_array[k] - G0_OH_ref)
                        end
                        ...
                        k_ER_fwd[k] = (kB*T/h) * exp(-dG_ER_act/(kB*T))
                    ...
                    ddG_ER[j,:] = -1 .* (0.25 .* gamma_BEP .* (z_CO_CO .* t_CO_array_for_CO_order[j,:] + z_CO_OH .* t_OH_array_for_CO_order[j,:]))

                    k_ER_fwd_cov_dep[j,:] = k_ER_fwd[:] .* exp.(-ddG_ER[j,:] ./ (kB*T))

                    ER_rate[j,:] = k_ER_fwd_cov_dep[j,:] .* t_CO_array_for_CO_order[j,:] .* a_OH_array[i]

                    total_rate[j,:] = ER_rate[j,:]
        ...
        if orderType == "OH order"
            ...
            for i = 1:length(P_CO_array)
                ...
                k_ER_fwd = zeros(length(E_array))
                ddG_ER = zeros(length(a_OH_array),length(E_array))
                k_ER_fwd_cov_dep = zeros(length(a_OH_array),length(E_array))
                ER_rate = zeros(length(a_OH_array),length(E_array))
                ...
                for j = 1:length(a_OH_array)
                    for k = 1:length(E_array)
                        ...
                        #define potential dependent barriers at zero coverage
                        if dG_ER_act_ref - beta_ER * (E_array[k] - G0_OH_ref - E_OH_ref) < 0
                            dG_ER_act = 0
                        else
                            dG_ER_act = dG_ER_act_ref - beta_ER * (E_array[k] - G0_OH_ref - E_OH_ref)
                        end
                    ...
                    ddG_ER[j,:] = -1 .* (0.25 .* gamma_BEP .* (z_CO_CO .* t_CO_array_for_OH_order[j,:] + z_CO_OH .* t_OH_array_for_OH_order[j,:]))

                    k_ER_fwd_cov_dep[j,:] = k_ER_fwd[:] .* exp.(-ddG_ER[j,:] ./ (kB*T))
                
                    ER_rate[j,:] = k_ER_fwd_cov_dep[j,:] .* t_CO_array_for_OH_order[j,:] .* a_OH_array[j]
    ...
    function getNegLogLikelihood(params)
        ...
        model_output = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")
        model_output_OH_order = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    function runOptimization(objective)
        ...
        # bnds
        bnds = (dG0_CO_g_to_CO_ref_bounds, dG0_OH_aq_to_OH_ref_bounds, beta_ER_bounds, z_CO_CO_bounds, z_CO_OH_bounds, z_OH_CO_bounds, z_OH_OH_bounds)
    ...
    function simulateWithFittedParams(fitted_params)
        ...
        beta_ER = fitted_params[3]
        z_CO_CO = fitted_params[4]
        z_CO_OH = fitted_params[5]
        z_OH_CO = fitted_params[6]
        z_OH_OH = fitted_params[7]

        alpha_and_CO_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array_simulated, P_CO_array, a_OH_array, "CO order")

        OH_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array_OH_simulated, P_CO_array, a_OH_array, "OH order")
        ...
        return t_CO_CO_order, t_CO_OH_order, t_OH_CO_order, t_OH_OH_order, alpha_250_fitted, alpha_500_fitted, alpha_1000_fitted, CO_order_250_fitted, CO_order_500_fitted, CO_order_1000_fitted, OH_order_fitted,
    ...
    function getChiSquared(fitted_params)
        ...
        alpha_and_CO_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")

        OH_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, beta_ER, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    println()
    XLSX.openxlsx("Ag75Pd25_single_site_ER_CO_differential_with_lat_int_coverages_rates.xlsx", mode = "rw") do f
    ...
    XLSX.openxlsx("Ag75Pd25_single_site_ER_CO_differential_with_lat_int_observables.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "Model potential (V vs RHE)"
        sheet["A2",dim=1] = E_array_simulated
        sheet["B1"] = "Experimental potential (V vs RHE)"
    ...
    XLSX.openxlsx("Ag75Pd25_single_site_ER_CO_differential_with_lat_int_local_minima.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "dG_CO"
        sheet["B1"] = "dG_OH"
        sheet["C1"] = "beta_ER"
...
function runOverall()
    ...
    # ER symmetry factor
    beta_ER_prior_mean = 0.5
    beta_ER_bounds = (0.0, 1.0)
