...
function controlFitting(global_opt_iter, local_opt_iter, priors, bounds)
    ...
    function getNegLogLikelihood(params)
        ...
        model_output_OH_order = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    function simulateWithFittedParams(fitted_params)
        ...
        OH_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, E_array_OH_simulated, P_CO_array, a_OH_array, "OH order")
    ...
    function getChiSquared(fitted_params)
        ...
        OH_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    println()
    XLSX.openxlsx("Ag90Pd10_single_site_LH_CO_differential_coverages_rates.xlsx", mode = "rw") do f
    ...
    XLSX.openxlsx("Ag90Pd10_single_site_LH_CO_differential_observables.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "Model potential (V vs RHE)"
        sheet["A2",dim=1] = E_array_simulated
        sheet["B1"] = "Experimental potential (V vs RHE)"
        ...
        sheet["CE2"] = std_error
        sheet["CF1"] = "Model potential (V vs SHE)"
        sheet["CF2",dim=1] = E_array_OH_simulated
    end
    ### Write the optimization trajectory here
    XLSX.openxlsx("Ag90Pd10_single_site_LH_CO_differential_local_minima.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "dG_CO"
        sheet["B1"] = "dG_OH"
        sheet["C1"] = "Likelihood"
