...
function controlFitting(global_opt_iter, local_opt_iter, priors, bounds)
    ...
    z_CO_CO_prior_mean = priors[4]
    z_CO_OH_prior_mean = priors[5]
    z_OH_CO_prior_mean = priors[6]
    z_OH_OH_prior_mean = priors[7]

    dG0_CO_g_to_CO_ref_bounds = bounds[1]
    dG0_OH_aq_to_OH_S1_ref_bounds = bounds[2]
    dG0_OH_aq_to_OH_S2_ref_bounds = bounds[3]
    z_CO_CO_bounds = bounds[4]
    z_CO_OH_bounds = bounds[5]
    z_OH_CO_bounds = bounds[6]
    z_OH_OH_bounds = bounds[7]

    function simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array, P_CO_array, a_OH_array, orderType)
            ...
            for i = 1:length(a_OH_array)
                ...
                for j = 1:length(P_CO_array)
                    for k = 1:length(E_array)
                        ...
                        G0_OH_S2 = G0_OH_S2_ref - (E_array[k] - Eref)
                        ...
                        function f!(fvec,x)
                            ...
                            driving_CO = (K_CO_0 * exp(-(z_CO_CO * t_CO + z_CO_OH * (t_OH_S1 + t_OH_S2))/(kB*T)) * P_CO_array[j])
                            driving_OH_S1 = (K_OH_S1_0 * exp(-(z_OH_CO * t_CO + z_OH_OH * (t_OH_S1 + t_OH_S2))/(kB*T)) * a_OH_array[i])
                            driving_OH_S2 = (K_OH_S2_0 * exp(-(z_OH_CO * t_CO + z_OH_OH * (t_OH_S1 + t_OH_S2))/(kB*T)) * a_OH_array[i])
                        ...
                        function j!(jvec, x)
                            ...
                            jvec[1,3] = (z_CO_OH/(kB*T) * driving_CO * (site_1 - t_CO) - z_OH_OH/(kB*T) * driving_OH_S1 * t_CO)
                            ...
                            jvec[2,3] = (z_OH_OH/(kB*T) * driving_OH_S1 * (site_1 - t_OH_S1) - z_CO_OH/(kB*T) * driving_CO * t_OH_S1)
                            jvec[3,1] = z_OH_CO/(kB*T) * driving_OH_S2 * ((1 - site_1) - t_OH_S2)
                            jvec[3,2] = z_OH_OH/(kB*T) * driving_OH_S2 * ((1 - site_1) - t_OH_S2)
                            jvec[3,3] = denom_S2 + z_OH_OH/(kB*T) * driving_OH_S2 * ((1 - site_1) - t_OH_S2)
                    ...
                    ddG_bi[j,:] = -1 .* (gamma_BEP .* (z_OH_CO .* t_CO_array_for_CO_order[j,:] + z_OH_OH .* t_OH_S1_array_for_CO_order[j,:] + z_OH_OH .* t_OH_S2_array_for_CO_order[j,:]) + 0.25 .* gamma_BEP .* (z_CO_CO .* t_CO_array_for_CO_order[j,:] + z_CO_OH .* t_OH_S1_array_for_CO_order[j,:] + z_CO_OH .* t_OH_S2_array_for_CO_order[j,:]))
            ...
            for i = 1:length(P_CO_array)
                ...
                for j = 1:length(a_OH_array)
                    for k = 1:length(E_array)
                        ...
                        G0_OH_S2 = G0_OH_S2_ref - (E_array[k] - E_OH_ref)
                        #define barriers at reference potential and zero coverage
                        if alpha_BEP + gamma_BEP * (0.67 - 0.25 * G0_CO_ref - G0_OH_S2_ref) < 0
                            dG_bi_act_ref = 0
                        else
                        ...
                        function f!(fvec,x)
                            ...
                            t_OH_S2 = x[3]
                            # Group the "driving force" terms for each species (i.e. the numerators in the expressions for the coverages of each species)
                            driving_CO = (K_CO_0 * exp(-(z_CO_CO * t_CO + z_CO_OH * (t_OH_S1 + t_OH_S2))/(kB*T)) * P_CO_array[i])
                            driving_OH_S1 = (K_OH_S1_0 * exp(-(z_OH_CO * t_CO + z_OH_OH * (t_OH_S1 + t_OH_S2))/(kB*T)) * a_OH_array[j])
                            driving_OH_S2 = (K_OH_S2_0 * exp(-(z_OH_CO * t_CO + z_OH_OH * (t_OH_S1 + t_OH_S2))/(kB*T)) * a_OH_array[j])
                    ...
                    ddG_bi[j,:] = -1 .* (gamma_BEP .* (z_OH_CO .* t_CO_array_for_OH_order[j,:] + z_OH_OH .* t_OH_S1_array_for_OH_order[j,:] + z_OH_OH .* t_OH_S2_array_for_OH_order[j,:]) + 0.25 .* gamma_BEP .* (z_CO_CO .* t_CO_array_for_OH_order[j,:] + z_CO_OH .* t_OH_S1_array_for_OH_order[j,:] + z_CO_OH .* t_OH_S2_array_for_OH_order[j,:]))
    ...
    function getNegLogLikelihood(params)
        ...
        model_output = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")
        model_output_OH_order = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    function runOptimization(objective)
        ...
        # bnds
        bnds = (dG0_CO_g_to_CO_ref_bounds, dG0_OH_aq_to_OH_S1_ref_bounds, dG0_OH_aq_to_OH_S2_ref_bounds, z_CO_CO_bounds, z_CO_OH_bounds, z_OH_CO_bounds, z_OH_OH_bounds)
    ...
    function simulateWithFittedParams(fitted_params)
        ...
        z_CO_CO = fitted_params[4]
        z_CO_OH = fitted_params[5]
        z_OH_CO = fitted_params[6]
        z_OH_OH = fitted_params[7]

        alpha_and_CO_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array_simulated, P_CO_array, a_OH_array, "CO order")

        OH_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array_OH_simulated, P_CO_array, a_OH_array, "OH order")
    ...
    function getChiSquared(fitted_params)
        ...
        alpha_and_CO_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")

        OH_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    println()
    XLSX.openxlsx("Ag90Pd10_dual_site_BLH_CO_differential_with_lat_int_coverages_rates.xlsx", mode = "rw") do f
    ...
    XLSX.openxlsx("Ag90Pd10_dual_site_BLH_CO_differential_with_lat_int_observables.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "Model potential (V vs RHE)"
        sheet["A2",dim=1] = E_array_simulated
        sheet["B1"] = "Experimental potential (V vs RHE)"
        ...
        sheet["CI1"] = "Parameter fits"
        sheet["CI2",dim=1] = fitted_params
        sheet["CJ1"] = "Std Error from Finite Difference"
        sheet["CJ2"] = std_error
        sheet["CK1"] = "Model potential (V vs SHE)"
        sheet["CK2",dim=1] = E_array_OH_simulated
    end
    ### Write the optimization trajectory here
    XLSX.openxlsx("Ag90Pd10_dual_site_BLH_CO_differential_with_lat_int_local_minima.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "dG_CO"
        sheet["B1"] = "dG_OH_S1"
        sheet["C1"] = "dG_OH_S2"
