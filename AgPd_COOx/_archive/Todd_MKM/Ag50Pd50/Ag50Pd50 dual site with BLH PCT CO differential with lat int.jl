...
function controlFitting(global_opt_iter, local_opt_iter, priors, bounds)
    ...
    dG0_OH_aq_to_OH_S1_ref_prior_mean = priors[2]
    dG0_OH_aq_to_OH_S2_ref_prior_mean = priors[3]
    mu_prior_mean = priors[4]
    beta_bi_prior_mean = priors[5]
    z_CO_CO_prior_mean = priors[6]
    z_CO_OH_prior_mean = priors[7]
    z_OH_CO_prior_mean = priors[8]
    z_OH_OH_prior_mean = priors[9]

    dG0_CO_g_to_CO_ref_bounds = bounds[1]
    dG0_OH_aq_to_OH_S1_ref_bounds = bounds[2]
    dG0_OH_aq_to_OH_S2_ref_bounds = bounds[3]
    mu_bounds = bounds[4]
    beta_bi_bounds = bounds[5]
    z_CO_CO_bounds = bounds[6]
    z_CO_OH_bounds = bounds[7]
    z_OH_CO_bounds = bounds[8]
    z_OH_OH_bounds = bounds[9]

    function simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array, P_CO_array, a_OH_array, orderType)
        ...
        G0_OH_S1_ref = dG0_OH_aq_to_OH_S1_ref
        G0_OH_S2_ref = dG0_OH_aq_to_OH_S2_ref

        site_1 = 0.5

        # Initial condition first
        initialConditionFirst = [0.75 * site_1, 0.01 * site_1, 0.1 * (1 - site_1)]
        ...
        if orderType == "CO order"
            ...
            t_OH_S1_CO_order_dict = Dict()
            t_OH_S2_CO_order_dict = Dict()
            total_rate_dict = Dict()
            for i = 1:length(a_OH_array)
                t_CO_array_for_CO_order = zeros(length(P_CO_array),length(E_array))
                t_OH_S1_array_for_CO_order = zeros(length(P_CO_array),length(E_array))
                t_OH_S2_array_for_CO_order = zeros(length(P_CO_array),length(E_array))
                k_bi_fwd = zeros(length(E_array))
                ddG_bi = zeros(length(P_CO_array),length(E_array))
                k_bi_fwd_cov_dep = zeros(length(P_CO_array),length(E_array))
                bi_rate = zeros(length(P_CO_array),length(E_array))
                ...
                for j = 1:length(P_CO_array)
                    for k = 1:length(E_array)
                        ...
                        if k == 1
                            ...
                            IC = [t_CO_array_for_CO_order[j,k-1], t_OH_S1_array_for_CO_order[j,k-1], t_OH_S2_array_for_CO_order[j,k-1]] 
                        end

                        G0_CO = G0_CO_ref 
                        G0_OH_S1 = G0_OH_S1_ref - (E_array[k] - Eref)
                        G0_OH_S2 = G0_OH_S2_ref - mu * (E_array[k] - Eref)
                        #define barriers at reference potential and zero coverage
                        if alpha_BEP + gamma_BEP * (0.67 - 0.25 * G0_CO_ref - G0_OH_S2_ref) < 0
                            dG_bi_act_ref = 0
                        else
                            dG_bi_act_ref = alpha_BEP + gamma_BEP * (0.67 - 0.25 * G0_CO_ref - G0_OH_S2_ref)
                        end

                        #define potential dependent barriers at zero coverage
                        if dG_bi_act_ref - (1 - mu) * beta_bi * E_array[k] < 0
                            dG_bi_act = 0
                        else
                            dG_bi_act = dG_bi_act_ref - (1 - mu) * beta_bi * E_array[k]
                        end

                        # Get the equilibrium constants in the limit of zero coverage
                        K_CO_0 = exp(-G0_CO/(kB*T))
                        K_OH_S1_0 = exp(-G0_OH_S1/(kB*T))
                        K_OH_S2_0 = exp(-G0_OH_S2/(kB*T))

                        # And the potential-dependent forward ER rate constant
                        k_bi_fwd[k] = (kB*T/h) * exp(-dG_bi_act/(kB*T))
                        ...
                        function f!(fvec,x)
                            ...
                            driving_CO = (K_CO_0 * exp(-(z_CO_CO * t_CO + z_CO_OH * t_OH_S1 + z_CO_OH * mu * t_OH_S2)/(kB*T)) * P_CO_array[j])
                            driving_OH_S1 = (K_OH_S1_0 * exp(-(z_OH_CO * t_CO + z_OH_OH * t_OH_S1 + z_OH_OH * mu * t_OH_S2)/(kB*T)) * a_OH_array[i])
                            driving_OH_S2 = (K_OH_S2_0 * exp(-(z_OH_CO * mu * t_CO + z_OH_OH * mu * t_OH_S1 + z_OH_OH * mu^2 * t_OH_S2)/(kB*T)) * a_OH_array[i])

                            # All denominators corresponding to each site type are the same.
                            denom_S1 = 1 + driving_CO + driving_OH_S1
                            denom_S2 = 1 + driving_OH_S2

                            # Set coverage terms equal to zero and solve for roots
                            fvec[1] = (t_CO * denom_S1) - site_1 * driving_CO
                            fvec[2] = (t_OH_S1 * denom_S1) - site_1 * driving_OH_S1
                            fvec[3] = (t_OH_S2 * denom_S2) - (1 - site_1 ) * driving_OH_S2
                        ...
                        function j!(jvec, x)
                            ...
                            jvec[1,1] = denom_S1 + z_CO_CO/(kB*T) * driving_CO * (site_1 - t_CO) - z_OH_CO/(kB*T) * driving_OH_S1 * t_CO
                            jvec[1,2] = z_CO_OH/(kB*T) * driving_CO * (site_1 - t_CO) - z_OH_OH/(kB*T) * driving_OH_S1 * t_CO
                            jvec[1,3] = mu * (z_CO_OH/(kB*T) * driving_CO * (site_1 - t_CO) - z_OH_OH/(kB*T) * driving_OH_S1 * t_CO)
                            jvec[2,1] = z_OH_CO/(kB*T) * driving_OH_S1 * (site_1 - t_OH_S1) - z_CO_CO/(kB*T) * driving_CO * t_OH_S1
                            jvec[2,2] = denom_S1 + z_OH_OH/(kB*T) * driving_OH_S1 * (site_1 - t_OH_S1) - z_CO_OH/(kB*T) * driving_CO * t_OH_S1
                            jvec[2,3] = mu * (z_OH_OH/(kB*T) * driving_OH_S1 * (site_1 - t_OH_S1) - z_CO_OH/(kB*T) * driving_CO * t_OH_S1)
                            jvec[3,1] = mu * z_OH_CO/(kB*T) * driving_OH_S2 * ((1 - site_1) - t_OH_S2)
                            jvec[3,2] = mu * z_OH_OH/(kB*T) * driving_OH_S2 * ((1 - site_1) - t_OH_S2)
                            jvec[3,3] = denom_S2 + mu^2 * z_OH_OH/(kB*T) * driving_OH_S2 * ((1 - site_1) - t_OH_S2)
                        ...
                        if solution[2] < 0
                            t_OH_S1_array_for_CO_order[j,k] = 10 ^ (-20)
                        elseif solution[2] > 1
                            t_OH_S1_array_for_CO_order[j,k] = 0.99999999999999999999
                        else
                            t_OH_S1_array_for_CO_order[j,k] = solution[2]
                        end
                        if solution[3] < 0
                            t_OH_S2_array_for_CO_order[j,k] = 10 ^ (-20)
                        elseif solution[3] > 1
                            t_OH_S2_array_for_CO_order[j,k] = 0.99999999999999999999
                        else
                            t_OH_S2_array_for_CO_order[j,k] = solution[3]
                        end

                    end

                    ddG_bi[j,:] = -1 .* (mu .* gamma_BEP .* (z_OH_CO .* t_CO_array_for_CO_order[j,:] + z_OH_OH .* t_OH_S1_array_for_CO_order[j,:] + mu .* z_OH_OH .* t_OH_S2_array_for_CO_order[j,:]) + 0.25 .* gamma_BEP .* (z_CO_CO .* t_CO_array_for_CO_order[j,:] + z_CO_OH .* t_OH_S1_array_for_CO_order[j,:] + z_CO_OH .* mu .* t_OH_S2_array_for_CO_order[j,:]))

                    k_bi_fwd_cov_dep[j,:] = k_bi_fwd[:] .* exp.(-ddG_bi[j,:] ./ (kB*T))

                    bi_rate[j,:] = k_bi_fwd_cov_dep[j,:] .* t_CO_array_for_CO_order[j,:] .* t_OH_S2_array_for_CO_order[j,:]

                    total_rate[j,:] = bi_rate[j,:]
                
                    ln_rate_for_CO_order[j,:] = log.(abs.(total_rate[j,:]))

                end
                t_CO_CO_order_dict[i] = t_CO_array_for_CO_order
                t_OH_S1_CO_order_dict[i] = t_OH_S1_array_for_CO_order
                t_OH_S2_CO_order_dict[i] = t_OH_S2_array_for_CO_order
        ...
        if orderType == "OH order"
            ...
            t_OH_S1_OH_order_dict = Dict()
            t_OH_S2_OH_order_dict = Dict()
            total_rate_dict = Dict()
            ...
            for i = 1:length(P_CO_array)
                ...
                t_OH_S1_array_for_OH_order = zeros(length(a_OH_array),length(E_array))
                t_OH_S2_array_for_OH_order = zeros(length(a_OH_array),length(E_array))
                k_bi_fwd = zeros(length(E_array))
                ddG_bi = zeros(length(a_OH_array),length(E_array))
                k_bi_fwd_cov_dep = zeros(length(a_OH_array),length(E_array))
                bi_rate = zeros(length(a_OH_array),length(E_array))
                ...
                for j = 1:length(a_OH_array)
                    for k = 1:length(E_array)
                        ...
                        if k == 1
                            ...
                            IC = [t_CO_array_for_OH_order[j,k-1], t_OH_S1_array_for_OH_order[j,k-1], t_OH_S2_array_for_OH_order[j,k-1]] 
                        end

                        G0_CO = G0_CO_ref
                        G0_OH_S1 = G0_OH_S1_ref - (E_array[k] - E_OH_ref)
                        G0_OH_S2 = G0_OH_S2_ref - mu * (E_array[k] - E_OH_ref)
                        #define barriers at reference potential and zero coverage
                        if alpha_BEP + gamma_BEP * (0.67 - 0.25 * G0_CO_ref - G0_OH_S2_ref) < 0
                            dG_bi_act_ref = 0
                        else
                        ...
                        #define potential dependent barriers at zero coverage
                        if dG_bi_act_ref - (1 - mu) * beta_bi * (E_array[k] - E_OH_ref) < 0
                            dG_bi_act = 0
                        else
                            dG_bi_act = dG_bi_act_ref - (1 - mu) * beta_bi * (E_array[k] - E_OH_ref)
                        end
                        ...
                        function f!(fvec,x)
                            ...
                            t_OH_S2 = x[3]
                            # Group the "driving force" terms for each species (i.e. the numerators in the expressions for the coverages of each species)
                            driving_CO = (K_CO_0 * exp(-(z_CO_CO * t_CO + z_CO_OH * t_OH_S1 + z_CO_OH * mu * t_OH_S2)/(kB*T)) * P_CO_array[i])
                            driving_OH_S1 = (K_OH_S1_0 * exp(-(z_OH_CO * t_CO + z_OH_OH * t_OH_S1 + z_OH_OH * mu * t_OH_S2)/(kB*T)) * a_OH_array[j])
                            driving_OH_S2 = (K_OH_S2_0 * exp(-(z_OH_CO * mu * t_CO + z_OH_OH * mu * t_OH_S1 + z_OH_OH * mu^2 * t_OH_S2)/(kB*T)) * a_OH_array[j])
                        ...
                        if solution[2] < 0
                            t_OH_S1_array_for_OH_order[j,k] = 10 ^ (-20)
                        elseif solution[2] > 1
                            t_OH_S1_array_for_OH_order[j,k] = 0.99999999999999999999
                        else
                            t_OH_S1_array_for_OH_order[j,k] = solution[2]
                        end
                        if solution[3] < 0
                            t_OH_S2_array_for_OH_order[j,k] = 10 ^ (-20)
                        elseif solution[3] > 1
                            t_OH_S2_array_for_OH_order[j,k] = 0.99999999999999999999
                        else
                            t_OH_S2_array_for_OH_order[j,k] = solution[3]
                        end
                    end
                    
                    ddG_bi[j,:] = -1 .* (mu .* gamma_BEP .* (z_OH_CO .* t_CO_array_for_OH_order[j,:] + z_OH_OH .* t_OH_S1_array_for_OH_order[j,:] + mu .* z_OH_OH .* t_OH_S2_array_for_OH_order[j,:]) + 0.25 .* gamma_BEP .* (z_CO_CO .* t_CO_array_for_OH_order[j,:] + z_CO_OH .* t_OH_S1_array_for_OH_order[j,:] + z_CO_OH .* mu .* t_OH_S2_array_for_OH_order[j,:]))

                    k_bi_fwd_cov_dep[j,:] = k_bi_fwd[:] .* exp.(-ddG_bi[j,:] ./ (kB*T))
                
                    bi_rate[j,:] = k_bi_fwd_cov_dep[j,:] .* t_CO_array_for_OH_order[j,:] .* t_OH_S2_array_for_OH_order[j,:]
                ...
                t_OH_S1_OH_order_dict[i] = t_OH_S1_array_for_OH_order
                t_OH_S2_OH_order_dict[i] = t_OH_S2_array_for_OH_order
                total_rate_dict[i] = total_rate
            end 
        end
        # Output the fully 2D arrays (coverages, alpha, etc) across the range of CO pressures
        if orderType == "CO order"
            return alpha_250, alpha_500, alpha_1000, CO_order_250, CO_order_500, CO_order_1000, t_CO_CO_order_dict, t_OH_S1_CO_order_dict, t_OH_S2_CO_order_dict, total_rate_dict
        elseif orderType == "OH order"
            return OH_order, t_CO_OH_order_dict, t_OH_S1_OH_order_dict, t_OH_S2_OH_order_dict, total_rate_dict
        end
    end
    ...
    function getNegLogLikelihood(params)
        ...
        dG0_OH_aq_to_OH_S1_ref = params[2]
        dG0_OH_aq_to_OH_S2_ref = params[3]
        mu = params[4]
        beta_bi = params[5]
        z_CO_CO = params[6]
        z_CO_OH = params[7]
        ...
        model_output = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")
        model_output_OH_order = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    function runOptimization(objective)
        ...
        initialGuess = [dG0_CO_g_to_CO_ref_prior_mean, 
                        dG0_OH_aq_to_OH_S1_ref_prior_mean,
                        dG0_OH_aq_to_OH_S2_ref_prior_mean,
                        mu_prior_mean,
                        beta_bi_prior_mean,
                        z_CO_CO_prior_mean, 
                        z_CO_OH_prior_mean, 
        ...
        # bnds
        bnds = (dG0_CO_g_to_CO_ref_bounds, dG0_OH_aq_to_OH_S1_ref_bounds, dG0_OH_aq_to_OH_S2_ref_bounds, mu_bounds, beta_bi_bounds, z_CO_CO_bounds, z_CO_OH_bounds, z_OH_CO_bounds, z_OH_OH_bounds)
    ...
    function simulateWithFittedParams(fitted_params)
        ...
        dG0_OH_aq_to_OH_S1_ref = fitted_params[2]
        dG0_OH_aq_to_OH_S2_ref = fitted_params[3]
        mu = fitted_params[4]
        beta_bi = fitted_params[5]
        z_CO_CO = fitted_params[6]
        z_CO_OH = fitted_params[7]
        z_OH_CO = fitted_params[8]
        z_OH_OH = fitted_params[9]

        alpha_and_CO_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array_simulated, P_CO_array, a_OH_array, "CO order")

        OH_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, E_array_OH_simulated, P_CO_array, a_OH_array, "OH order")
        ...
        t_OH_S1_CO_order = alpha_and_CO_order_simulatedOutput[8]
        t_OH_S1_OH_order = OH_order_simulatedOutput[3]

        t_OH_S2_CO_order = alpha_and_CO_order_simulatedOutput[9]
        t_OH_S2_OH_order = OH_order_simulatedOutput[4]

        total_rate_CO_order = alpha_and_CO_order_simulatedOutput[10]
        total_rate_OH_order = OH_order_simulatedOutput[5]

        return t_CO_CO_order, t_CO_OH_order, t_OH_S1_CO_order, t_OH_S1_OH_order, t_OH_S2_CO_order, t_OH_S2_OH_order, alpha_250_fitted, alpha_500_fitted, alpha_1000_fitted, CO_order_250_fitted, CO_order_500_fitted, CO_order_1000_fitted, OH_order_fitted,
        total_rate_CO_order, total_rate_OH_order
    ...
    function getChiSquared(fitted_params)
        ...
        alpha_and_CO_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")

        OH_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_S1_ref, dG0_OH_aq_to_OH_S2_ref, mu, beta_bi, z_CO_CO, z_CO_OH, z_OH_CO, z_OH_OH, expt_she_potential, P_CO_array, a_OH_array, "OH order")
    ...
    t_OH_S1_CO_order = fitted_data[3]
    t_OH_S1_0_1_250_CO_order = t_OH_S1_CO_order[1][1,:]
    t_OH_S1_1_250_CO_order = t_OH_S1_CO_order[1][2,:]
    t_OH_S1_10_250_CO_order = t_OH_S1_CO_order[1][3,:]
    t_OH_S1_100_250_CO_order = t_OH_S1_CO_order[1][4,:]
    t_OH_S1_0_1_500_CO_order = t_OH_S1_CO_order[2][1,:]
    t_OH_S1_1_500_CO_order = t_OH_S1_CO_order[2][2,:]
    t_OH_S1_10_500_CO_order = t_OH_S1_CO_order[2][3,:]
    t_OH_S1_100_500_CO_order = t_OH_S1_CO_order[2][4,:]
    t_OH_S1_0_1_1000_CO_order = t_OH_S1_CO_order[3][1,:]
    t_OH_S1_1_1000_CO_order = t_OH_S1_CO_order[3][2,:]
    t_OH_S1_10_1000_CO_order = t_OH_S1_CO_order[3][3,:]
    t_OH_S1_100_1000_CO_order = t_OH_S1_CO_order[3][4,:]
    t_OH_S1_OH_order = fitted_data[4]
    t_OH_S1_0_1_250_OH_order = t_OH_S1_OH_order[1][1,:]
    t_OH_S1_0_1_500_OH_order = t_OH_S1_OH_order[1][2,:]
    t_OH_S1_0_1_1000_OH_order = t_OH_S1_OH_order[1][3,:]
    t_OH_S1_1_250_OH_order = t_OH_S1_OH_order[2][1,:]
    t_OH_S1_1_500_OH_order = t_OH_S1_OH_order[2][2,:]
    t_OH_S1_1_1000_OH_order = t_OH_S1_OH_order[2][3,:]
    t_OH_S1_10_250_OH_order = t_OH_S1_OH_order[3][1,:]
    t_OH_S1_10_500_OH_order = t_OH_S1_OH_order[3][2,:]
    t_OH_S1_10_1000_OH_order = t_OH_S1_OH_order[3][3,:]
    t_OH_S1_100_250_OH_order = t_OH_S1_OH_order[4][1,:]
    t_OH_S1_100_500_OH_order = t_OH_S1_OH_order[4][2,:]
    t_OH_S1_100_1000_OH_order = t_OH_S1_OH_order[4][3,:]
    t_OH_S2_CO_order = fitted_data[5]
    t_OH_S2_0_1_250_CO_order = t_OH_S2_CO_order[1][1,:]
    t_OH_S2_1_250_CO_order = t_OH_S2_CO_order[1][2,:]
    t_OH_S2_10_250_CO_order = t_OH_S2_CO_order[1][3,:]
    t_OH_S2_100_250_CO_order = t_OH_S2_CO_order[1][4,:]
    t_OH_S2_0_1_500_CO_order = t_OH_S2_CO_order[2][1,:]
    t_OH_S2_1_500_CO_order = t_OH_S2_CO_order[2][2,:]
    t_OH_S2_10_500_CO_order = t_OH_S2_CO_order[2][3,:]
    t_OH_S2_100_500_CO_order = t_OH_S2_CO_order[2][4,:]
    t_OH_S2_0_1_1000_CO_order = t_OH_S2_CO_order[3][1,:]
    t_OH_S2_1_1000_CO_order = t_OH_S2_CO_order[3][2,:]
    t_OH_S2_10_1000_CO_order = t_OH_S2_CO_order[3][3,:]
    t_OH_S2_100_1000_CO_order = t_OH_S2_CO_order[3][4,:]
    t_OH_S2_OH_order = fitted_data[6]
    t_OH_S2_0_1_250_OH_order = t_OH_S2_OH_order[1][1,:]
    t_OH_S2_0_1_500_OH_order = t_OH_S2_OH_order[1][2,:]
    t_OH_S2_0_1_1000_OH_order = t_OH_S2_OH_order[1][3,:]
    t_OH_S2_1_250_OH_order = t_OH_S2_OH_order[2][1,:]
    t_OH_S2_1_500_OH_order = t_OH_S2_OH_order[2][2,:]
    t_OH_S2_1_1000_OH_order = t_OH_S2_OH_order[2][3,:]
    t_OH_S2_10_250_OH_order = t_OH_S2_OH_order[3][1,:]
    t_OH_S2_10_500_OH_order = t_OH_S2_OH_order[3][2,:]
    t_OH_S2_10_1000_OH_order = t_OH_S2_OH_order[3][3,:]
    t_OH_S2_100_250_OH_order = t_OH_S2_OH_order[4][1,:]
    t_OH_S2_100_500_OH_order = t_OH_S2_OH_order[4][2,:]
    t_OH_S2_100_1000_OH_order = t_OH_S2_OH_order[4][3,:]
    alpha_250_fitted = fitted_data[7]
    ...
    alpha_500_fitted = fitted_data[8]
    ...
    alpha_1000_fitted = fitted_data[9]
    ...
    CO_order_250_fitted = fitted_data[10]
    ...
    CO_order_500_fitted = fitted_data[11]
    ...
    CO_order_1000_fitted = fitted_data[12]
    ...
    OH_order_fitted = fitted_data[13]
    ...
    total_rate_CO_order = fitted_data[14]
    ...
    total_rate_OH_order = fitted_data[15]
    ...
    println()
    XLSX.openxlsx("Ag50Pd50_dual_site_BLH_PCT_CO_differential_with_lat_int_coverages_rates.xlsx", mode = "rw") do f
        ...
        sheet["P1"] = "Theta_OH_S1 at 0.1% CO, 0.25 M KOH - CO order"
        sheet["P2",dim=1] = t_OH_S1_0_1_250_CO_order
        sheet["Q1"] = "Theta_OH_S1 at 1% CO, 0.25 M KOH - CO order"
        sheet["Q2",dim=1] = t_OH_S1_1_250_CO_order
        sheet["R1"] = "Theta_OH_S1 at 10% CO, 0.25 M KOH - CO order"
        sheet["R2",dim=1] = t_OH_S1_10_250_CO_order
        sheet["S1"] = "Theta_OH_S1 at 100% CO, 0.25 M KOH - CO order"
        sheet["S2",dim=1] = t_OH_S1_100_250_CO_order
        sheet["T1"] = "Theta_OH_S1 at 0.1% CO, 0.5 M KOH - CO order"
        sheet["T2",dim=1] = t_OH_S1_0_1_500_CO_order
        sheet["U1"] = "Theta_OH_S1 at 1% CO, 0.5 M KOH - CO order"
        sheet["U2",dim=1] = t_OH_S1_1_500_CO_order
        sheet["V1"] = "Theta_OH_S1 at 10% CO, 0.5 M KOH - CO order"
        sheet["V2",dim=1] = t_OH_S1_10_500_CO_order
        sheet["W1"] = "Theta_OH_S1 at 100% CO, 0.5 M KOH - CO order"
        sheet["W2",dim=1] = t_OH_S1_100_500_CO_order
        sheet["X1"] = "Theta_OH_S1 at 0.1% CO, 1 M KOH - CO order"
        sheet["X2",dim=1] = t_OH_S1_0_1_1000_CO_order
        sheet["Y1"] = "Theta_OH_S1 at 1% CO, 1 M KOH - CO order"
        sheet["Y2",dim=1] = t_OH_S1_1_1000_CO_order
        sheet["Z1"] = "Theta_OH_S1 at 10% CO, 1 M KOH - CO order"
        sheet["Z2",dim=1] = t_OH_S1_10_1000_CO_order
        sheet["AA1"] = "Theta_OH_S1 at 100% CO, 1 M KOH - CO order"
        sheet["AA2",dim=1] = t_OH_S1_100_1000_CO_order
        sheet["AB1"] = "Theta_OH_S2 at 0.1% CO, 0.25 M KOH - CO order"
        sheet["AB2",dim=1] = t_OH_S2_0_1_250_CO_order
        sheet["AC1"] = "Theta_OH_S2 at 1% CO, 0.25 M KOH - CO order"
        sheet["AC2",dim=1] = t_OH_S2_1_250_CO_order
        sheet["AD1"] = "Theta_OH_S2 at 10% CO, 0.25 M KOH - CO order"
        sheet["AD2",dim=1] = t_OH_S2_10_250_CO_order
        sheet["AE1"] = "Theta_OH_S2 at 100% CO, 0.25 M KOH - CO order"
        sheet["AE2",dim=1] = t_OH_S2_100_250_CO_order
        sheet["AF1"] = "Theta_OH_S2 at 0.1% CO, 0.5 M KOH - CO order"
        sheet["AF2",dim=1] = t_OH_S2_0_1_500_CO_order
        sheet["AG1"] = "Theta_OH_S2 at 1% CO, 0.5 M KOH - CO order"
        sheet["AG2",dim=1] = t_OH_S2_1_500_CO_order
        sheet["AH1"] = "Theta_OH_S2 at 10% CO, 0.5 M KOH - CO order"
        sheet["AH2",dim=1] = t_OH_S2_10_500_CO_order
        sheet["AI1"] = "Theta_OH_S2 at 100% CO, 0.5 M KOH - CO order"
        sheet["AI2",dim=1] = t_OH_S2_100_500_CO_order
        sheet["AJ1"] = "Theta_OH_S2 at 0.1% CO, 1 M KOH - CO order"
        sheet["AJ2",dim=1] = t_OH_S2_0_1_1000_CO_order
        sheet["AK1"] = "Theta_OH_S2 at 1% CO, 1 M KOH - CO order"
        sheet["AK2",dim=1] = t_OH_S2_1_1000_CO_order
        sheet["AL1"] = "Theta_OH_S2 at 10% CO, 1 M KOH - CO order"
        sheet["AL2",dim=1] = t_OH_S2_10_1000_CO_order
        sheet["AM1"] = "Theta_OH_S2 at 100% CO, 1 M KOH - CO order"
        sheet["AM2",dim=1] = t_OH_S2_100_1000_CO_order
        sheet["AN1"] = "Total Rate at 0.1% CO, 0.25 M KOH - CO order"
        sheet["AN2",dim=1] = total_rate_0_1_250_CO_order
        sheet["AO1"] = "Total Rate at 1% CO, 0.25 M KOH - CO order"
        sheet["AO2",dim=1] = total_rate_1_250_CO_order
        sheet["AP1"] = "Total Rate at 10% CO, 0.25 M KOH - CO order"
        sheet["AP2",dim=1] = total_rate_10_250_CO_order
        sheet["AQ1"] = "Total Rate at 100% CO, 0.25 M KOH - CO order"
        sheet["AQ2",dim=1] = total_rate_100_250_CO_order
        sheet["AR1"] = "Total Rate at 0.1% CO, 0.5 M KOH - CO order"
        sheet["AR2",dim=1] = total_rate_0_1_500_CO_order
        sheet["AS1"] = "Total Rate at 1% CO, 0.5 M KOH - CO order"
        sheet["AS2",dim=1] = total_rate_1_500_CO_order
        sheet["AT1"] = "Total Rate at 10% CO, 0.5 M KOH - CO order"
        sheet["AT2",dim=1] = total_rate_10_500_CO_order
        sheet["AU1"] = "Total Rate at 100% CO, 0.5 M KOH - CO order"
        sheet["AU2",dim=1] = total_rate_100_500_CO_order
        sheet["AV1"] = "Total Rate at 0.1% CO, 1 M KOH - CO order"
        sheet["AV2",dim=1] = total_rate_0_1_1000_CO_order
        sheet["AW1"] = "Total Rate at 1% CO, 1 M KOH - CO order"
        sheet["AW2",dim=1] = total_rate_1_1000_CO_order
        sheet["AX1"] = "Total Rate at 10% CO, 1 M KOH - CO order"
        sheet["AX2",dim=1] = total_rate_10_1000_CO_order
        sheet["AY1"] = "Total Rate at 100% CO, 1 M KOH - CO order"
        sheet["AY2",dim=1] = total_rate_100_1000_CO_order
        sheet["AZ1"] = "Theta_CO at 0.1% CO, 0.25 M KOH - OH order"
        sheet["AZ2",dim=1] = t_CO_0_1_250_OH_order
        sheet["BA1"] = "Theta_CO at 0.1% CO, 0.5 M KOH - OH order"
        sheet["BA2",dim=1] = t_CO_0_1_500_OH_order
        sheet["BB1"] = "Theta_CO at 0.1% CO, 1 M KOH - OH order"
        sheet["BB2",dim=1] = t_CO_0_1_1000_OH_order
        sheet["BC1"] = "Theta_CO at 1% CO, 0.25 M KOH - OH order"
        sheet["BC2",dim=1] = t_CO_1_250_OH_order
        sheet["BD1"] = "Theta_CO at 1% CO, 0.5 M KOH - OH order"
        sheet["BD2",dim=1] = t_CO_1_500_OH_order
        sheet["BE1"] = "Theta_CO at 1% CO, 1 M KOH - OH order"
        sheet["BE2",dim=1] = t_CO_1_1000_OH_order
        sheet["BF1"] = "Theta_CO at 10% CO, 0.25 M KOH - OH order"
        sheet["BF2",dim=1] = t_CO_10_250_OH_order
        sheet["BG1"] = "Theta_CO at 10% CO, 0.5 M KOH - OH order"
        sheet["BG2",dim=1] = t_CO_10_500_OH_order
        sheet["BH1"] = "Theta_CO at 10% CO, 1 M KOH - OH order"
        sheet["BH2",dim=1] = t_CO_10_1000_OH_order
        sheet["BI1"] = "Theta_CO at 100% CO, 0.25 M KOH - OH order"
        sheet["BI2",dim=1] = t_CO_100_250_OH_order
        sheet["BJ1"] = "Theta_CO at 100% CO, 0.5 M KOH - OH order"
        sheet["BJ2",dim=1] = t_CO_100_500_OH_order
        sheet["BK1"] = "Theta_CO at 100% CO, 1 M KOH - OH order"
        sheet["BK2",dim=1] = t_CO_100_1000_OH_order
        sheet["BL1"] = "Theta_OH_S1 at 0.1% CO, 0.25 M KOH - OH order"
        sheet["BL2",dim=1] = t_OH_S1_0_1_250_OH_order
        sheet["BM1"] = "Theta_OH_S1 at 0.1% CO, 0.5 M KOH - OH order"
        sheet["BM2",dim=1] = t_OH_S1_0_1_500_OH_order
        sheet["BN1"] = "Theta_OH_S1 at 0.1% CO, 1 M KOH - OH order"
        sheet["BN2",dim=1] = t_OH_S1_0_1_1000_OH_order
        sheet["BO1"] = "Theta_OH_S1 at 1% CO, 0.25 M KOH - OH order"
        sheet["BO2",dim=1] = t_OH_S1_1_250_OH_order
        sheet["BP1"] = "Theta_OH_S1 at 1% CO, 0.5 M KOH - OH order"
        sheet["BP2",dim=1] = t_OH_S1_1_500_OH_order
        sheet["BQ1"] = "Theta_OH_S1 at 1% CO, 1 M KOH - OH order"
        sheet["BQ2",dim=1] = t_OH_S1_1_1000_OH_order
        sheet["BR1"] = "Theta_OH_S1 at 10% CO, 0.25 M KOH - OH order"
        sheet["BR2",dim=1] = t_OH_S1_10_250_OH_order
        sheet["BS1"] = "Theta_OH_S1 at 10% CO, 0.5 M KOH - OH order"
        sheet["BS2",dim=1] = t_OH_S1_10_500_OH_order
        sheet["BT1"] = "Theta_OH_S1 at 10% CO, 1 M KOH - OH order"
        sheet["BT2",dim=1] = t_OH_S1_10_1000_OH_order
        sheet["BU1"] = "Theta_OH_S1 at 100% CO, 0.25 M KOH - OH order"
        sheet["BU2",dim=1] = t_OH_S1_100_250_OH_order
        sheet["BV1"] = "Theta_OH_S1 at 100% CO, 0.5 M KOH - OH order"
        sheet["BV2",dim=1] = t_OH_S1_100_500_OH_order
        sheet["BW1"] = "Theta_OH_S1 at 100% CO, 1 M KOH - OH order"
        sheet["BW2",dim=1] = t_OH_S1_100_1000_OH_order
        sheet["BX1"] = "Theta_OH_S2 at 0.1% CO, 0.25 M KOH - OH order"
        sheet["BX2",dim=1] = t_OH_S2_0_1_250_OH_order
        sheet["BY1"] = "Theta_OH_S2 at 0.1% CO, 0.5 M KOH - OH order"
        sheet["BY2",dim=1] = t_OH_S2_0_1_500_OH_order
        sheet["BZ1"] = "Theta_OH_S2 at 0.1% CO, 1 M KOH - OH order"
        sheet["BZ2",dim=1] = t_OH_S2_0_1_1000_OH_order
        sheet["CA1"] = "Theta_OH_S2 at 1% CO, 0.25 M KOH - OH order"
        sheet["CA2",dim=1] = t_OH_S2_1_250_OH_order
        sheet["CB1"] = "Theta_OH_S2 at 1% CO, 0.5 M KOH - OH order"
        sheet["CB2",dim=1] = t_OH_S2_1_500_OH_order
        sheet["CC1"] = "Theta_OH_S2 at 1% CO, 1 M KOH - OH order"
        sheet["CC2",dim=1] = t_OH_S2_1_1000_OH_order
        sheet["CD1"] = "Theta_OH_S2 at 10% CO, 0.25 M KOH - OH order"
        sheet["CD2",dim=1] = t_OH_S2_10_250_OH_order
        sheet["CE1"] = "Theta_OH_S2 at 10% CO, 0.5 M KOH - OH order"
        sheet["CE2",dim=1] = t_OH_S2_10_500_OH_order
        sheet["CF1"] = "Theta_OH_S2 at 10% CO, 1 M KOH - OH order"
        sheet["CF2",dim=1] = t_OH_S2_10_1000_OH_order
        sheet["CG1"] = "Theta_OH_S2 at 100% CO, 0.25 M KOH - OH order"
        sheet["CG2",dim=1] = t_OH_S2_100_250_OH_order
        sheet["CH1"] = "Theta_OH_S2 at 100% CO, 0.5 M KOH - OH order"
        sheet["CH2",dim=1] = t_OH_S2_100_500_OH_order
        sheet["CI1"] = "Theta_OH_S2 at 100% CO, 1 M KOH - OH order"
        sheet["CI2",dim=1] = t_OH_S2_100_1000_OH_order
        sheet["CJ1"] = "Total Rate at 0.1% CO, 0.25 M KOH - OH order"
        sheet["CJ2",dim=1] = total_rate_0_1_250_OH_order
        sheet["CK1"] = "Total Rate at 0.1% CO, 0.5 M KOH - OH order"
        sheet["CK2",dim=1] = total_rate_0_1_500_OH_order
        sheet["CL1"] = "Total Rate at 0.1% CO, 1 M KOH - OH order"
        sheet["CL2",dim=1] = total_rate_0_1_1000_OH_order
        sheet["CM1"] = "Total Rate at 1% CO, 0.25 M KOH - OH order"
        sheet["CM2",dim=1] = total_rate_1_250_OH_order
        sheet["CN1"] = "Total Rate at 1% CO, 0.5 M KOH - OH order"
        sheet["CN2",dim=1] = total_rate_1_500_OH_order
        sheet["CO1"] = "Total Rate at 1% CO, 1 M KOH - OH order"
        sheet["CO2",dim=1] = total_rate_1_1000_OH_order
        sheet["CP1"] = "Total Rate at 10% CO, 0.25 M KOH - OH order"
        sheet["CP2",dim=1] = total_rate_10_250_OH_order
        sheet["CQ1"] = "Total Rate at 10% CO, 0.5 M KOH - OH order"
        sheet["CQ2",dim=1] = total_rate_10_500_OH_order
        sheet["CR1"] = "Total Rate at 10% CO, 1 M KOH - OH order"
        sheet["CR2",dim=1] = total_rate_10_1000_OH_order
        sheet["CS1"] = "Total Rate at 100% CO, 0.25 M KOH - OH order"
        sheet["CS2",dim=1] = total_rate_100_250_OH_order
        sheet["CT1"] = "Total Rate at 100% CO, 0.5 M KOH - OH order"
        sheet["CT2",dim=1] = total_rate_100_500_OH_order
        sheet["CU1"] = "Total Rate at 100% CO, 1 M KOH - OH order"
        sheet["CU2",dim=1] = total_rate_100_1000_OH_order
        sheet["CV1"] = "Model potential (V vs SHE)"
        sheet["CV2",dim=1] = E_array_OH_simulated
    end
    #### Write all of the observables and errors here, as well as the fitted params and covariance etc
    XLSX.openxlsx("Ag50Pd50_dual_site_BLH_PCT_CO_differential_with_lat_int_observables.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "Model potential (V vs RHE)"
        sheet["A2",dim=1] = E_array_simulated
        sheet["B1"] = "Experimental potential (V vs RHE)"
        ...
        sheet["CK1"] = "Parameter fits"
        sheet["CK2",dim=1] = fitted_params
        sheet["CL1"] = "Std Error from Finite Difference"
        sheet["CL2"] = std_error
        sheet["CM1"] = "Model potential (V vs SHE)"
        sheet["CM2",dim=1] = E_array_OH_simulated
    end
    ### Write the optimization trajectory here
    XLSX.openxlsx("Ag50Pd50_dual_site_BLH_PCT_CO_differential_with_lat_int_local_minima.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "dG_CO"
        sheet["B1"] = "dG_OH_S1"
        sheet["C1"] = "dG_OH_S2"
        ...
        sheet["H2",dim=1] = local_mins[:,8]
        sheet["I2",dim=1] = local_mins[:,9]
        sheet["J2",dim=1] = local_mins[:,10]
    end
end
...
function runOverall()
    ...
    #################################
    # 1. CO(g) + (*) <-> CO(*)
    dG0_CO_g_to_CO_ref_prior_mean = -0.8
    dG0_CO_g_to_CO_ref_bounds = (-1.2, 0.0)
    # 2. OH-(aq) + (*) <-> OH(*) + e-
    dG0_OH_aq_to_OH_S1_ref_prior_mean = 0.4
    dG0_OH_aq_to_OH_S1_ref_bounds = (0.0, 1.2)
    # 2. OH-(aq) + (#) <-> OH(#) + e-
    dG0_OH_aq_to_OH_S2_ref_prior_mean = 0.2
    dG0_OH_aq_to_OH_S2_ref_bounds = (0.0, 1.2)
    # partial charge transfer coefficient
    mu_prior_mean = 0.5
    mu_bounds = (0.0, 1.0)
    # bi symmetry factor
    beta_bi_prior_mean = 0.5
    beta_bi_bounds = (0.0, 1.0)
    # CO-CO interaction param
    z_CO_CO_prior_mean = 0.5
    z_CO_CO_bounds = (0.0, 1.0)
    # CO-OH interaction param
    z_CO_OH_prior_mean = 0.5
    z_CO_OH_bounds = (0.0, 1.0)
    # OH-CO interaction param
    z_OH_CO_prior_mean = 0.5
    z_OH_CO_bounds = (0.0, 1.0)
    # OH-OH interaction param
    z_OH_OH_prior_mean = 0.5
    ...
    bounds = [dG0_CO_g_to_CO_ref_bounds, 
              dG0_OH_aq_to_OH_S1_ref_bounds,
              dG0_OH_aq_to_OH_S2_ref_bounds,
              mu_bounds,
              beta_bi_bounds,
              z_CO_CO_bounds,
              z_CO_OH_bounds,
