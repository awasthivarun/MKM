...
# Load and parse data
with open('import_Ag50Pd50_replicates.pkl', 'rb') as f:
...
def plot_model_fits(trace, ppc, consolidated=False):
    ...
    figsize_CO   = (1.5 + 1.5 * n_C, 1.0 + 1.5 * (n_P - 1))
    ...
    def plot_grid(var_fit, nrows, ncols, figsize, ylabel, title_text, is_CO_grid=False, is_OH_grid=False, consolidated=False):
        ...
        axes_2d = np.atleast_2d(axes) if axes.ndim < 2 else axes
        ...
        fig.supylabel(ylabel, fontweight='bold', fontsize=label_size + 2, x=0.02)
        ...
        if consolidated and var_fit in ['alpha', 'delta_CO']:
            ...
            for i, C_KOH in enumerate(C_KOH_list):
                ...
                loop_limit = len(P_CO_list) if var_fit == 'alpha' else len(P_CO_list) - 1
                for pressure_idx in range(loop_limit):
                    P1 = P_CO_list[pressure_idx]
                    P2 = P_CO_list[pressure_idx + 1] if var_fit == 'delta_CO' else None
                    cond_key = (C_KOH, P1)
                    
                    exp_mean = alpha_exp[cond_key] if var_fit == 'alpha' else delta_CO_exp[cond_key]
                    E_exp = E_exp_map[cond_key] if var_fit == 'alpha' else E_CO_exp[cond_key]
                    ...
                    label_str = f'{P1} atm' if var_fit == 'alpha' else f'{P1} - {P2} atm'
                ...
                ax.text(0.95 if var_fit == 'alpha' else 0.05, 0.95, f'{C_KOH} M KOH', transform=ax.transAxes, fontsize=label_size, fontweight='bold', va='top', ha='right' if var_fit == 'alpha' else 'left')
                ticks = [-0.5, 0, 0.5] if var_fit == 'alpha' else [0.0, 0.5, 1.0]
                ax.set_yticks(ticks); ax.set_yticklabels(ticks, fontweight='bold')
                for label in ax.get_xticklabels(): label.set_fontweight('bold')
                
                if var_fit == 'alpha' and i == n_C - 1:
                    ax.legend(handles=legend_handles, loc='lower left', frameon=False, prop={'size': label_size, 'weight': 'bold'})

            plt.tight_layout()
            bottom_pad = 0.1 if var_fit == 'alpha' else 0.14
            plt.subplots_adjust(right=0.92, top=0.91, left=0.12, bottom=bottom_pad)
            if var_fit == 'delta_CO':
        ...
        if consolidated and var_fit == 'delta_OH':
            ...
            ax.set_xticks(ticks); ax.set_xticklabels(ticks, fontweight='bold')
            plt.tight_layout(); plt.subplots_adjust(right=0.92, top=0.9, left=0.15, bottom=0.14)
        ...
        for i, C_KOH in enumerate(C_KOH_list):
            for j, P_CO in enumerate(P_CO_list):
                if is_OH_grid and i > 0: continue
                if is_CO_grid and j >= n_P - 1: continue

                ax = axes_2d[0, j] if is_OH_grid else axes_2d[j, i]
                cond_key = (C_KOH_list[1] if is_OH_grid else C_KOH, P_CO)

                if var_fit == 'log_rate':
                    exp_mean, exp_sd, E_exp = log_rate_exp[cond_key], log_rate_SD_exp[cond_key], E_exp_map[cond_key]
                elif var_fit == 'alpha':
                    exp_mean, exp_sd, E_exp = alpha_exp[cond_key], alpha_SD_exp[cond_key], E_exp_map[cond_key]
                elif var_fit == 'rate':
                    exp_mean, exp_sd, E_exp = rate_exp[cond_key], rate_SD_exp[cond_key], E_exp_map[cond_key]
                elif var_fit == 'delta_OH':
                    exp_mean, exp_sd, E_exp = delta_OH_exp[cond_key], delta_OH_SD_exp[cond_key], E_OH_exp[cond_key]
                elif var_fit == 'delta_CO':
                    exp_mean, exp_sd, E_exp = delta_CO_exp[cond_key], delta_CO_SD_exp[cond_key], E_CO_exp[cond_key]
                
                model_slice, E_model = get_model_slice(var_fit, C_KOH if not is_OH_grid else None, P_CO, is_ppc=is_ppc)
                ...
                if j == 0 and not is_OH_grid: ax.set_title(f'{C_KOH} M KOH', fontsize=label_size, fontweight='bold')
                if is_OH_grid: ax.set_title(f'{P_CO} atm', fontsize=label_size, fontweight='bold')
                if i == n_C - 1 and not is_OH_grid:
                    label_right = f'{P_CO} atm' if not is_CO_grid else f'{P_CO_list[j]} - {P_CO_list[j+1]} atm'
        ...
        plt.tight_layout(); plt.subplots_adjust(right=0.92, top=top_adj, left=0.12, bottom=bottom_adj); plt.show()
    ...
    if consolidated:
        ...
        plot_grid('delta_OH', 1, n_P, figsize_OH, "Order (OH)", "OH Reaction Order", is_OH_grid=True)
        plot_grid('delta_CO', n_P - 1, n_C, figsize_CO, "Order (CO)", "CO Reaction Order", is_CO_grid=True)
...
def plot_drc(model, trace, perturb_vars, perturb_labels=None, var_types=None):
    ...
    fig, axes = plt.subplots(nrows=n_P, ncols=n_C, figsize=figsize, sharex='col', sharey=True)
    axes_2d = np.atleast_2d(axes) # CHANGED: Safety wrapper
...
def observables(log_rate):
    ...
    rate = pm.StudentT('rate', nu=3, mu=rate_model, sigma=sigma_model, observed=rate_obs_matrix)
    return rate