...
# Load and parse data
with open(f'import_{alloy}_KOH.pkl', 'rb') as f:
    data = pickle.load(f)
    experiments_interp = data['truncated']
    untruncated_data = data['untruncated']
        ...
        for i, C_KOH in enumerate(C_KOH_list):
            for j, P_CO in enumerate(P_CO_list):
                ...
                if is_1x4:
                    ...
                    exp_mean = untruncated_data[cond_key]['delta_OH']
                    E_exp = untruncated_data[cond_key]['shared_E_OH']
                    model_slice, E_model = get_model_slice(var_track, None, P_CO, is_ppc)           
                elif is_3x4:
                    ax = axes[j, i]
                    cond_key = (C_KOH, P_CO)
                    exp_mean = untruncated_data[cond_key]['delta_CO']
                    E_exp = untruncated_data[cond_key]['shared_E_CO']
                    model_slice, E_model = get_model_slice(var_track, C_KOH, P_CO, is_ppc)
                else:
                    ax = axes[j, i]
                    cond_key = (C_KOH, P_CO)
                    if var_fit == 'alpha':
                        exp_mean = untruncated_data[cond_key]['alpha']
                        E_exp = untruncated_data[cond_key]['E']
                        ...
                        exp_mean = untruncated_data[cond_key]['untruncated_rate']
                        E_exp = untruncated_data[cond_key]['untruncated_E']    
                    model_slice, E_model = get_model_slice(var_track, C_KOH, P_CO, is_ppc)
...
def observables(log_rate_model):
    ...
    sigma_rel = pm.HalfNormal('sigma_rel', sigma=0.2)
    rate = pm.Normal('rate', mu=rate_model, sigma=rate_model*sigma_rel, observed=rate_obs)
