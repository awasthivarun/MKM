...
function getExperimentalData(material)
    ...
    cd("C:\\Users\\twhit\\My Drive\\CU_PhDResearch\\Holewinski Lab Files\\AgPd CO paper\\manifold data and models\\New CO data for fitting")
...
function controlFitting(global_opt_iter, local_opt_iter, priors, bounds)
    ...
    function getNegLogLikelihood(params)
        ...
        model_output = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")
        model_output_OH_order = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, expt_rhe_potential, P_CO_array, a_OH_array, "OH order")
    ...
    function runOptimization(objective)
        ...
        # bnds
        bnds = (dG0_CO_g_to_CO_ref_bounds, dG0_OH_aq_to_OH_ref_bounds)
    ...
    function simulateWithFittedParams(fitted_params)
        ...
        alpha_and_CO_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, E_array_simulated, P_CO_array, a_OH_array, "CO order")

        OH_order_simulatedOutput = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, E_array_simulated, P_CO_array, a_OH_array, "OH order")
    ...
    function getChiSquared(fitted_params)
        ...
        alpha_and_CO_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, expt_rhe_potential, P_CO_array, a_OH_array, "CO order")

        OH_order_chi_sq = simulateMacrokineticObservables(dG0_CO_g_to_CO_ref, dG0_OH_aq_to_OH_ref, expt_rhe_potential, P_CO_array, a_OH_array, "OH order")
    ...
    cd("C:\\Users\\twhit\\My Drive\\CU_PhDResearch\\Holewinski Lab Files\\AgPd CO paper\\manifold data and models\\differential models\\Ag75Pd25")
    ...
    println()
    XLSX.openxlsx("Ag75Pd25_single_site_LH_CO_differential_coverages_rates_v1.xlsx", mode = "rw") do f
    ...
    XLSX.openxlsx("Ag75Pd25_single_site_LH_CO_differential_observables_v1.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "Model potential (V vs RHE)"
        sheet["A2",dim=1] = E_array_simulated
        sheet["B1"] = "Experimental potential (V vs RHE)"
        ...
        sheet["CD1"] = "Parameter fits"
        sheet["CD2",dim=1] = fitted_params
        sheet["CE1"] = "Std Error from Finite Difference"
        sheet["CE2"] = std_error
    end
    ### Write the optimization trajectory here
    XLSX.openxlsx("Ag75Pd25_single_site_LH_CO_differential_local_minima_v1.xlsx", mode = "rw") do f
        sheet = f[1]
        sheet["A1"] = "dG_CO"
        sheet["B1"] = "dG_OH"
        sheet["C1"] = "Likelihood"
...
function runOverall()
    ...
    #################################
    # 1. CO(g) + (*) <-> CO(*)
    dG0_CO_g_to_CO_ref_prior_mean = -0.05
