...
def build_model_data(
    ...
    if selected_replicates.empty:
        raise ValueError(
            "Selected replicate data are empty."
        )
    ...
    np.testing.assert_allclose(
        ...
        atol=1e-12,
        err_msg=(
            "ln_rate is inconsistent with rate_s_inv."
        ),
    )
    ...
    if duplicate_observations.any():
        raise ValueError(
            "Duplicate replicate observations "
            "exist at the same experimental state."
        )
    ...
    if np.any(
        potential_counts.to_numpy()
        != 1
    ):
        raise ValueError(
            "A condition/grid index maps to "
            "multiple potentials."
        )
    ...
    if np.any(
        point_counts.to_numpy()
        < 1
    ):
        raise ValueError(
            "A model point has no observations."
        )
