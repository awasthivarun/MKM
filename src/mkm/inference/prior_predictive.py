...
def _get_group(
    ...
    try:
        ...
        raise ValueError(
            f"Prior-predictive result does not "
            f"contain group '{group_name}'."
        ) from error


def _flatten_draw_dimensions(
    values,
):
    values = np.asarray(
        values
    ...
    if values.ndim < 2:
        raise ValueError(
            "Expected at least chain and draw "
            "dimensions."
        )
...
def _summarize_draws(
    ...
    values = _flatten_draw_dimensions(
        values
    )
...
def sample_prior_predictive(
    ...
    if draws < 2:
        raise ValueError(
            "Prior-predictive sampling requires "
            "at least two draws."
        )
...
def summarize_prior_parameters(
    prior_predictive,
):
    prior = _get_group(
        prior_predictive,
...
def summarize_prior_model_points(
    prior_predictive,
    model_points,
):
    prior = _get_group(
    ...
    for name in variables:
        ...
        if (
            ...
            raise ValueError(
                f"Prior variable '{name}' does "
                "not align with model points."
            )
...
def summarize_prior_observations(
    prior_predictive,
    observations,
):
    ...
    for name in candidate_names:
        ...
        if (
            ...
            raise ValueError(
                f"Prior-predictive variable "
                f"'{name}' does not align with "
                "observations."
            )
