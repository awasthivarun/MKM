...
def get_observation_material_index(
...
def add_material_log_rate_likelihood(
    ...
    sigma_prior_median = float(
        sigma_prior_median
    )

    sigma_prior_log_sd = float(
        sigma_prior_log_sd
    )
    ...
    if (
        ...
        or sigma_prior_median <= 0
    ):
        raise ValueError(
            "sigma_prior_median must be finite "
            "and positive."
        )
    ...
    if (
        ...
        or sigma_prior_log_sd <= 0
    ):
        raise ValueError(
            "sigma_prior_log_sd must be finite "
            "and positive."
        )
    ...
    observed_ln_rate = np.asarray(
        inputs.observation_ln_rate,
        dtype=float,
    )

    ln_rate_model = pt.as_tensor_variable(
        ln_rate_model
    )
