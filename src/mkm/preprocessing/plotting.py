...
def _get_truncation_row(
    ...
    if len(rows) != 1:
        raise ValueError(
            f"Expected exactly one truncation record for "
            f"{material}, {C_KOH_M} M KOH, "
...
def _plot_rate_like_grid(
    ...
    for row, CO_fraction in enumerate(CO_values):
        for col, C_KOH_M in enumerate(KOH_values):
            ...
            full_condition = _get_condition_data(
                full_replicates,
                material,
                C_KOH_M,
                CO_fraction,
            ...
            selected_condition = _get_condition_data(
                selected_replicates,
                material,
                C_KOH_M,
                CO_fraction,
            ...
            truncation_row = _get_truncation_row(
                truncation,
                material,
                C_KOH_M,
                CO_fraction,
            ...
            cutoff_E = truncation_row[
                "retained_min_E_V_SHE"
            ]

            original_min_E = truncation_row[
            ...
            for replicate in config["replicates"]:
                full_curve = full_condition[
                    full_condition["replicate"]
                    == replicate
                ].sort_values("E_V_SHE")

                selected_curve = selected_condition[
                    selected_condition["replicate"]
                    == replicate
                ].sort_values("E_V_SHE")
                ...
                if not selected_curve.empty:
                    ax.plot(
                        selected_curve["E_V_SHE"],
                        selected_curve[replicate_column],
                        linewidth=1.2,
                        alpha=0.45,
                    )

            potential = summary_condition[
                "E_V_SHE"
            ].to_numpy()

            mean = summary_condition[
                mean_column
            ].to_numpy()

            sd = summary_condition[
                sd_column
            ].to_numpy()
...
def _plot_alpha_grid(
    ...
    for row, CO_fraction in enumerate(CO_values):
        for col, C_KOH_M in enumerate(KOH_values):
            ...
            replicate_condition = _get_condition_data(
                selected_replicates,
                material,
                C_KOH_M,
                CO_fraction,
    ...
    fig.suptitle(
        f"{material} — Transfer Coefficient",
        fontweight="bold",
    )
...
def plot_agpd_experimental_summary(
    ...
    return {
        "rate": rate_figure,
        "rate_logscale": rate_logscale_figure,
        "alpha": alpha_figure,
    }


def plot_agpd_oh_order(
    delta_OH,
    material,
    config,
):
    ...
    fig.tight_layout(
        rect=(0.04, 0.04, 1.0, 0.97)
    )

    return fig


def plot_agpd_co_order(
    delta_CO,
    material,
    config,
):
    ...
    for row, (lower_CO, upper_CO) in enumerate(
        CO_pairs
    ):
        for col, C_KOH_M in enumerate(
            KOH_values
        ):
            ax = axes[row, col]
