...
def _parse_co_mole_fraction(sheet_name, expected_values):
    ...
    if not sheet_name.endswith("% CO"):
        raise DataValidationError(
            f"Unexpected worksheet name '{sheet_name}'. "
            "Expected a name such as '0.1% CO'."
        )
    ...
    try:
        ...
        raise DataValidationError(
            f"Could not determine CO composition from "
...
def _parse_current_column(column_name, config):
    ...
    if len(parts) != 4:
        raise DataValidationError(
            f"Unexpected current-density column name '{column_name}'."
        )
    ...
    if not KOH_text.endswith("M"):
        raise DataValidationError(
            f"Could not determine KOH concentration from "
            f"column '{column_name}'."
        )

    if representation != "lnj":
        raise DataValidationError(
            f"Unexpected current representation in "
            f"column '{column_name}'."
        )

    if unit != "uA/cm2":
        raise DataValidationError(
            f"Unexpected current-density unit in "
            f"column '{column_name}'."
        )

    if not (
        replicate_text.startswith("(")
        and replicate_text.endswith(")")
    ):
        raise DataValidationError(
            f"Could not determine replicate from "
            f"column '{column_name}'."
        )
...
def load_agpd_workbook(path, config):
    ...
    if len(workbook.sheet_names) != expected_sheet_count:
        raise DataValidationError(
            f"Workbook '{path.name}' contains "
            f"{len(workbook.sheet_names)} worksheets; "
            f"expected {expected_sheet_count}."
        )
    ...
    for sheet_name in workbook.sheet_names:
        CO_mole_fraction = _parse_co_mole_fraction(
            sheet_name,
            config["CO_mole_fractions"],
        )

        if CO_mole_fraction in observed_CO:
            raise DataValidationError(
                f"Workbook '{path.name}' contains duplicate "
                f"CO composition {CO_mole_fraction}."
            )
        ...
        if sheet.columns[0] != "E (V vs SHE)":
            raise DataValidationError(
                f"Unexpected potential column '{sheet.columns[0]}' "
        ...
        for column_name in sheet.columns[1:]:
            C_KOH_M, replicate = _parse_current_column(
                column_name,
                config,
            )
            ...
            if condition in observed_conditions:
                raise DataValidationError(
                    f"Duplicate KOH/replicate combination "
                    f"{condition} in worksheet '{sheet_name}'."
                )
            ...
            validate_curve_arrays(
                potential,
                ln_current,
                expected_points=config["potential"].get(
                    "expected_points"
                ),
            )
        ...
        if observed_conditions != expected_conditions:
            ...
            raise DataValidationError(
                f"Unexpected condition structure in "
                f"worksheet '{sheet_name}'. "
    ...
    for material in config["materials"]:
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"Expected raw workbook not found: {path}"
            )
...
def _as_increasing_curve(potential, values, label):
    ...
    if direction == 0:
        raise DataValidationError(
            f"{label} has a non-monotonic potential vector. "
            "The relevant sweep must be selected before interpolation."
        )
...
def _build_analysis_grid(overlap_min, overlap_max, spacing, origin):
    if spacing <= 0:
        raise DataValidationError(
            "Analysis-grid spacing must be greater than zero."
        )

    if overlap_max <= overlap_min:
        raise DataValidationError(
            "Replicate potential ranges do not have a usable overlap."
        )

    tolerance = 1e-10

    first_index = int(
        np.ceil((overlap_min - origin - tolerance) / spacing)
    )
    last_index = int(
        np.floor((overlap_max - origin + tolerance) / spacing)
    )

    if last_index < first_index:
        raise DataValidationError(
            "Replicate potential overlap contains no points on the "
            "configured analysis grid."
        )

    analysis_grid_index = np.arange(
        first_index,
        last_index + 1,
        dtype=int,
    )
...
def _interpolate_log_current(
    ...
    if (
        analysis_potential[0] < potential[0] - tolerance
        or analysis_potential[-1] > potential[-1] + tolerance
    ):
        raise DataValidationError(
            f"Analysis grid for {condition} extends outside the "
            f"measured potential range of replicate '{replicate}'."
        )
    ...
    if not np.all(np.isfinite(interpolated)):
        raise DataValidationError(
            f"Interpolation produced non-finite values for "
...
def build_agpd_analysis_grid(standardized, config):
    ...
    for condition_values, condition_df in grouped:
        ...
        for replicate in config["replicates"]:
            replicate_df = (
                condition_df[
                    condition_df["replicate"] == replicate
                ]
        ...
        analysis_grid_index, analysis_potential = (
            _build_analysis_grid(
        ...
        for replicate in config["replicates"]:
            ...
            aligned_log_current[replicate] = (
                _interpolate_log_current(
...
def add_agpd_rates(analysis, config):
    ...
    electrons_per_CO = normalization[
        "electrons_per_CO_oxidation"
    ]
    site_charge_density = normalization[
        "site_charge_density_uC_cm2_Pd"
    ]

    if electrons_per_CO <= 0:
        raise DataValidationError(
            "Electrons per CO oxidation must be greater than zero."
        )

    if site_charge_density <= 0:
        raise DataValidationError(
            "Site charge density must be greater than zero."
        )
    ...
    result["ln_rate"] = (
        result["ln_j_uA_cm2"]
        - np.log(rate_denominator)
    )

    if not np.all(
        np.isfinite(result["rate_s_inv"])
    ):
        raise DataValidationError(
            "Rate conversion produced non-finite values."
        )
    ...
    if not np.all(
        np.isfinite(result["ln_rate"])
    ):
        raise DataValidationError(
            "Log-rate conversion produced non-finite values."
        )
...
def summarize_agpd_replicates(analysis_rates, config):
    ...
    replicate_counts = (
        analysis_rates
        .groupby(group_columns, sort=False)["replicate"]
        .nunique()
    )

    invalid_counts = replicate_counts[
        replicate_counts != expected_replicates
    ]

    if not invalid_counts.empty:
        raise DataValidationError(
            f"Expected {expected_replicates} replicates at every "
            f"AgPd analysis-grid point, but found "
            f"{len(invalid_counts)} point(s) with a different count."
        )
    ...
    if "alpha" in analysis_rates.columns:
        aggregations["alpha_mean"] = (
            "alpha",
            "mean",
        )
        aggregations["alpha_sd"] = (
            "alpha",
            "std",
        )
    ...
    if "alpha_mean" in summary.columns:
        required_finite_columns.extend(
            [
                "alpha_mean",
                "alpha_sd",
            ]
    ...
    if not np.all(
        ...
        raise DataValidationError(
            "Replicate summary contains non-finite values."
        )

    if not np.all(
        summary["rate_mean_s_inv"].to_numpy() > 0
    ):
        raise DataValidationError(
            "Mean rate must be strictly positive."
        )

    return summary


def _find_sustained_low_rate_cutoff(
    condition_rates,
    threshold_s_inv,
    ...
    pre_peak = grid_summary.iloc[
        : peak_position + 1
    ].copy()

    if require_all_replicates:
        threshold_rate = pre_peak[
            "min_rate_s_inv"
        ].to_numpy()
    else:
        threshold_rate = pre_peak[
    ...
    if len(valid_positions) == 0:
        raise DataValidationError(
            "No sustained low-potential region was found "
            f"above the rate threshold "
            f"{threshold_s_inv:g} s^-1 before the "
            "mean-rate maximum."
        )
    ...
    cutoff_row = pre_peak.iloc[
        cutoff_position
    ]

    peak_row = grid_summary.iloc[
        peak_position
    ]

    return {
        "cutoff_grid_index": int(
            cutoff_row[
                "analysis_grid_index"
            ]
        ),
        "cutoff_E_V_SHE": float(
            cutoff_row[
                "E_V_SHE"
            ]
        ),
...
def truncate_agpd_analysis(analysis_rates, config):
    ...
    low_config = truncation_config[
        "low_potential"
    ]
    high_config = truncation_config[
        "high_potential"
    ]
    ...
    if low_method not in {
        "none",
        "rate_threshold",
    }:
        raise DataValidationError(
            f"Unsupported low-potential truncation "
            f"method '{low_method}'."
        )

    if high_method != "none":
        raise DataValidationError(
            f"Unsupported high-potential truncation "
            f"method '{high_method}'."
        )
    ...
    grouped = analysis_rates.groupby(
        condition_columns,
        sort=False,
    )
    ...
    for condition_values, condition_data in grouped:
        ...
        original_min_E = float(
            grid_points[
                "E_V_SHE"
            ].min()
        )
        original_max_E = float(
            grid_points[
                "E_V_SHE"
            ].max()
        )

        original_n_points = len(
            grid_points
        )

        if low_method == "none":
            low_cut_grid_index = int(
                grid_points.iloc[0][
                    "analysis_grid_index"
                ]
            )
            ...
            if threshold_s_inv <= 0:
                raise DataValidationError(
                    "Low-potential rate threshold "
                    "must be greater than zero."
                )

            cutoff = (
                _find_sustained_low_rate_cutoff(
            ...
            low_cut_grid_index = cutoff[
                "cutoff_grid_index"
            ]
            low_cut_E = cutoff[
                "cutoff_E_V_SHE"
            ]
            peak_grid_index = cutoff[
                "peak_grid_index"
            ]
            peak_E = cutoff[
                "peak_E_V_SHE"
            ...
            peak_rate = cutoff[
                "peak_rate_mean_s_inv"
            ]

        retained = condition_data[
            condition_data[
                "analysis_grid_index"
            ]
            >= low_cut_grid_index
        ...
        retained_n_points = len(
            retained_grid
        )

        if retained_n_points < 3:
            raise DataValidationError(
                f"Truncation leaves fewer than three "
                f"potential points for condition "
                f"{condition}."
            )

        retained_min_E = float(
            retained_grid[
                "E_V_SHE"
            ].min()
        )
        retained_max_E = float(
            retained_grid[
                "E_V_SHE"
            ].max()
        )

        truncated_frames.append(
            retained
        )
        ...
        cutoff_records.append(
            {
                ...
                "original_min_E_V_SHE": (
                    original_min_E
                ),
                "original_max_E_V_SHE": (
                    original_max_E
                ),
                "retained_min_E_V_SHE": (
                    retained_min_E
                ),
                "retained_max_E_V_SHE": (
                    retained_max_E
                ),
                "peak_E_V_SHE": peak_E,
                "peak_rate_mean_s_inv": (
                ...
                "low_cut_analysis_grid_index": (
                    low_cut_grid_index
                ),
                "peak_analysis_grid_index": (
                    peak_grid_index
                ),
                "n_points_original": (
                    original_n_points
