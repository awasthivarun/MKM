...
ANALYSIS_SELECTED_PATH = (
    ANALYSIS_DIR
    / "AgPd_COOx_basic_selected.parquet"
)

SUMMARY_SELECTED_PATH = (
    ANALYSIS_DIR
...
def main():
    ...
    standardized = load_agpd_dataset(
        raw_dir=RAW_DIR,
        config=config,
    )

    analysis_full = build_agpd_analysis_grid(
        standardized=standardized,
        config=config,
    )

    analysis_full = add_agpd_rates(
    ...
    analysis_full.to_parquet(
        ANALYSIS_FULL_PATH,
        index=False,
    )

    analysis_selected.to_parquet(
        ANALYSIS_SELECTED_PATH,
        index=False,
    )

    summary_selected.to_parquet(
        SUMMARY_SELECTED_PATH,
        index=False,
    )

    truncation.to_parquet(
        TRUNCATION_PATH,
        index=False,
    )
    ...
    print(
        f"Standardized data: "
        f"{len(standardized)} rows"
    )

    print(
        f"Full analysis-grid data: "
        f"{len(analysis_full)} rows"
    )

    print(
        f"Selected analysis-grid data: "
        f"{len(analysis_selected)} rows"
    )

    print(
        f"Selected summary data: "
        f"{len(summary_selected)} rows"
    )

    print(
        f"Conditions truncated: "
    ...
    print(
        f"Saved standardized data to: "
        f"{STANDARDIZED_PATH}"
    )

    print(
        f"Saved full analysis data to: "
        f"{ANALYSIS_FULL_PATH}"
    )

    print(
        f"Saved selected analysis data to: "
        f"{ANALYSIS_SELECTED_PATH}"
    )

    print(
        f"Saved selected summary to: "
        f"{SUMMARY_SELECTED_PATH}"
    )

    print(
        f"Saved truncation metadata to: "
        f"{TRUNCATION_PATH}"
    )

    print(
        f"Saved OH-order data to: "
        f"{DELTA_OH_PATH}"
    )

    print(
        f"Saved CO-order data to: "
        f"{DELTA_CO_PATH}"
    )
